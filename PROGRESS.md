...
## v2 groundwork — Neglect Radar (first v2 provider)

- Neglect Radar shipped as the third `LeverageProvider` (`neglect-radar` in `src/lib/scoring/providers.ts`) — the pluggable interface absorbed it with zero engine changes, as designed. Categories that stop gathering points get a gentle additive boost on their open actions: +10 after 7 quiet days, +20 after 14, reason shown as "<category> quiet for N days". No penalties, no red — a nudge that never outranks a clearly higher-leverage action.
- Activity data flows in via `ScoringContext.categoryActivity` (optional — ranking degrades to v1 behavior while loading), fed by `useCategoryActivity` (last score event per category, 60-day lookback, cache invalidated by the same key prefix as totals). Categories with no recorded points anchor to their oldest open action, so a brand-new venture isn't "neglected" on day one.
- 8 new vitest tests (21 total): tier boundaries (6/7/13/14 days), fallback anchoring, silence without data, neighbor isolation, and two engine-level ranking scenarios. E2e harness gained a radar step — seeds a 20-day-old action in a quiet category via the API and asserts the "quiet for 20 days" reason renders — green in mock mode (8 steps), and it will run against the live project as part of `npm run e2e:live`.

## v2 groundwork — Reflection → Content Pipe

- Second v2 feature, no schema changes: tagged ledger entries are the material. Pure helpers in `src/lib/pipe.ts` (`groupByTag`, `reflectionMaterial`, `formatDigest` — 30-day material window, ready at 3 fresh entries) shared by both halves of the feature.
- Scoring side: `reflection-pipe` is the fourth `LeverageProvider` — when a tag has ≥3 fresh judgments, matching-category actions get +12 with reason "N fresh judgments tagged <category>" (the material is ready, so acting on the theme is high-leverage). Fed by `ScoringContext.reflectionMaterial` via `useReflectionMaterial`, derived from the existing ledger query cache; silent while loading, additive only.
- Content side: new Pipe page (`/app/pipe`, "Pipe" in nav) groups ledger entries by tag, richest fresh material first, with a quiet "ready to draft" badge and a "Copy digest" button that puts a markdown judgment log (situation/judgment/outcome per entry) on the clipboard for newsletter/script drafting. Untagged entries get a gentle pointer, not a warning.
- 12 new vitest tests (33 total). E2e harness gained a pipe step — seeds 3 tagged judgments + a matching action via API, asserts the ranking reason on Today, the Pipe grouping, and the actual clipboard contents — 9-step mock pass green, no console errors. Mock gained ledger_entries/ideas tables.

## Post-v2 — Code review pass (8-angle multi-agent review, fixes applied)

- Fixed (destructive-cleanup): the e2e harness deleted ALL of the account's rows by user_id — catastrophic with E2E_EMAIL on a real account. Cleanup now targets only ids created by the run (tracked at insert + stamped UI title), runs in `finally` so failed runs don't strand probe rows, and reports exact counts. Verified both success and injected-failure paths.
- Fixed (feature bugs): reflection-pipe tag↔category bridge is now case-insensitive ("Pricing" category meets "pricing" tag — keys lowercased in reflectionMaterial, lookup lowercased in the provider); neglect radar's activity map is now all-time instead of 60-day-windowed, so a dormant category with a freshly added action can no longer read as "not neglected" (ACTIVITY_LOOKBACK_DAYS removed).
- Fixed (harness on the live path): Node-side supabase client is proxy-aware via undici ProxyAgent (seeding/cleanup would have failed in the sandbox even with the allowlist fixed); the confirm-email signin click used a strict-mode-violating selector (two "Sign in" buttons) — now exact; probe categories/tags carry the run stamp so reruns against a reused account can't shift day/material counts; missing-browser launch failure now says to run `npx playwright install chromium`.
- Fixed (mock fidelity): missing/invalid JWT on protected tables now 401s like real PostgREST instead of returning empty 200s that masked auth regressions; malformed JSON bodies return 400 instead of crashing the process.
- Fixed (efficiency): Today no longer downloads the full ledger to count tags — useReflectionMaterial fetches only tags+created_at within the 30-day window.
- 35 tests green (2 new for the case bridge), 9-step mock e2e green, typecheck/lint clean.
